...
namespace pml {
  ...
  namespace Multinomial {
    ...
    inline Vector rand(const Vector &p, unsigned N) {
      ...
      gsl_ran_multinomial(Uniform::rnd_get_rng(), p.length(), N, p.data(), buf);
 ...
 *  2) Matrix inverse, pseudo inverse
 *  3) Matrix column / row operations
 *
 *  + Documentation
 *  + GitHub
 *
...
/* Discussions:
 *
 *  1) Distribution namespaces to classes ?
 *
 * */
