...
gp(:,:,3) = repmat(g1(:,1,3), [1 M2]) +  repmat(g2(:,1,3)', [M1 1]) + gammaln(gp(:,:,1)) - gp(:,:,1).*log(gp(:,:,2)) - gammaln(repmat(g1(:,1,1), [1 M2])) - gammaln( repmat(g2(:,1,1)', [M1 1])) + repmat(g1(:,1,1), [1 M2]).*log(repmat(g1(:,1,2), [1 M2])) +  repmat(g2(:,1,1)', [M1 1]).*log( repmat(g2(:,1,2)', [M1 1]));